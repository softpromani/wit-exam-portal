                            ...
                            @foreach ($examformdata as $formdata)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <span class="badge badge-warning"> {{$formdata->student->semester->semester_name
                                ?? ''}}</span>
                                            </td>
                                            <td>
                                                @php
                                                    $status = $formdata->payment_status;
                                                    // Double check against transactions if not marked as done
                                                    if ($status !== 'done') {
                                                        $hasOnlineSuccess = $formdata->transactions()
                                                            ->whereIn('status', ['success', 'completed'])
                                                            ->exists();
                                                        if ($hasOnlineSuccess) {
                                                            $status = 'done';
                                                            $formdata->update(['payment_status' => 'done']);
                                                        }
                                                    }
                                                @endphp
                                                ...
                                                @if($status == 'done')
                                                    ...
                                                    @php
                                                        $txn = $formdata->transactions()->where('status', 'success')->latest()->first();
                                                    @endphp
                                                    @if($txn)
                                                        <br><small class="text-muted">{{ ucfirst($txn->gateway) }}:
                                                            {{ $txn->transaction_id }}</small>
                                                    @endif
                                                @elseif($formdata->payment && $formdata->payment->payment_status == 'partial')
                                                    <span class="badge badge-warning">Partial Paid</span>
                                                    <br>₹ {{ $formdata->payment->paid_amount }}
                                                @else
                                                    <span class="badge badge-danger">Unpaid</span>
                                                @endif
                                            </td>
                                            <td>
                                                <a href="{{ route('admin.exam-form-show', $formdata->id) }}" target="_blank"><i
                                                        class="fa fa-eye text-primary"></i></a>
                                                @if($formdata->student->is_profile == 1)
                                                    <a href="#" class="ml-3" data-toggle="modal" data-target="#paymentModal"
                                                        data-id="{{ $formdata->id }}" id="paymentBtn"><i
                                                            class="fa fa-credit-card text-success"></i></a>
                                                @else
                ...
                <form action="{{ route('admin.fee-payment') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            @enderror
                            <div class="col-md-6 mb-3">
                                <label for="transaction_id">Transaction No / Reference ID</label>
                                <input type="text" class="form-control" name="transaction_id" placeholder="Enter Ref No">
                            </div>
...
@section('script_section')
    ...
    <script>
        $(document).on('click', '#paymentBtn, .payment-btn-dynamic', function (e) {
            e.preventDefault();
                        ...
                        if (response) {
                            ...
                            response.examsession.forEach(function (examsession, index) {
                                ...
                                if (examsession.payment_status == 'done') {
                                    paymentHtml = '<span class="badge badge-success">Paid</span>';
                                } else {
                                    paymentHtml = '<span class="badge badge-danger">Unpaid</span>';
                                }
                                ...
                                rows += '<tr>' +
                                    ...
                                    '<td>' + paymentHtml + '</td>' +
                                    '<td>' +
                                    '<a href="{{ url('admin/exam-form-show') }}/' + examsession.id + '" target="_blank"><i class="fa fa-eye text-primary"></i></a>' +
                                    '<a href="#" class="ml-3 payment-btn-dynamic" data-id="' + examsession.id + '"><i class="fa fa-credit-card text-success"></i></a>' +
                                    '</td>' +
                                    '</tr>';
