...
class ExamController extends Controller
{
    ...
    {
        $query = ExamSchedule::with(['subject', 'exam_session']);

        if ($request->filled('filter_session')) {
            $query->where('exam_session_id', $request->filter_session);
        }

        $examschedule = $query->orderBy('date', 'desc')->paginate(10);
    ...
    public function marksfeedList(Request $req)
    {
        ...
        if ($req->filled('examsession') && $req->filled('subject')) {
            ...
            $studentsData = ExamForm::where('session_id', $session_id)
                ...
                ->with([
                    'student',
                    'examfrom_has_subjects' => function ($query) use ($subject_id) {
